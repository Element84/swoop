...
def test_includes():
    includes = ['process.workflow', 'features[*].id', 'features[*].collection', 'features[0].assets.image']
    excludes = ['*']
    result = transform_payload(payload_1,includes,excludes)
        ...
        'features': [
            ...
            'assets': {
                'image': {
                    ...
                    'roles': ['data'], 'title': 'RGBIR COG tile', 'type': 'image/tiff; application=geotiff; profile=cloud-optimized'}
                }
            },
            {
                'id': 'a_item',
...
def test_inc_exc_1():
    includes = ['process.workflow', 'features[*].id', 'features[*].collection', 'features[0].properties.some']
...
def test_inc_exc_2():
    includes = ['process.workflow', 'features[*].id', 'features[*].collection', 'features[0].properties.a_value']
...
def test_inc_exc_3():
    includes = ['process.workflow', 'features[*].id', 'features[*].collection',
                'process.upload_options', 'process.tasks.copy-assets.drop_assets']
