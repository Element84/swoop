...
export default function() {
  const swoopApiRoot = http.get('http://{{.apiHost}}/');
  check(swoopApiRoot, {
    'status of SWOOP API root test was 200': (r) => r.status == 200
  });
  const swoopApiJobs = http.get('http://{{.apiHost}}/jobs/');
  check(swoopApiJobs, {
    'status of SWOOP API jobs endpoint (with swoop-db) test was 200': (r) => r.status == 200
  });
  const swoopApiJobPayload = http.get('http://{{.apiHost}}/jobs/2595f2da-81a6-423c-84db-935e6791046e/payload');
